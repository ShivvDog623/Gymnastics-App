...
from app.models.meet_info.meet_info import MeetDetailsResponse, MeetDetailsCreate, MeetDetails, MeetDetailsUpdate
...
def get_meet_details_by_id(meet_id: int, db: Session = Depends(get_db)):
    meet_details = db.query(MeetDetails).filter(MeetDetails.meet_id == meet_id).first()
    if not meet_details:
