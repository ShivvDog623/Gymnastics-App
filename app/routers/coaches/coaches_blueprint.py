...
def create_coach(coach_data: CoachCreate, db: Session = Depends(get_db)):
    if db.query(Coaches).filter(Coaches.coach_pro_number == coach_data.coach_pro_number).first():
        raise HTTPException(status_code=409, detail="Coach with this pro number already exists")
...
# Get coach by USAG Pro Number
@coach_router.get("/pro-number/{coach_pro_number}", response_model=CoachResponse)
def get_coach_by_pro_number(coach_pro_number: int, db: Session = Depends(get_db)):
    coach_info = db.query(Coaches).filter(Coaches.coach_pro_number == coach_pro_number).first()
    if not coach_info:
        raise HTTPException(status_code=404, detail="Coach not found")
    return coach_info
